...
**Needs Help**:
The ⁉️-labeled `checkboxes` element (a single checkbox, not a Yes/No choice) available on every Quick Pulse and Deep Check-in opening message — see ADR-0016 for why it's a checkbox rather than a reaction or a forced-choice question. Checking it alone is enough to raise a flag — a follow-up free-text note, sent as a threaded reply on the opening message before the Quick Pulse Answer Window closes, appends to the same flag rather than gating whether it fires (the same reply-in-thread shape as an Attendance Note). All flags raised in the same window are batched into one summary posted to the Wellbeing Reporting Channel at window close, never one post per flag.
_Avoid_: "needs_help" (the column name, not the concept)

**Wellbeing Disclosure Notice**:
A blockquoted line at the bottom of every Quick Pulse and Deep Check-in opening message, naming who sees survey results (the Wellbeing Reporting Channel's membership) and inviting the student to ask a trusted team adult if they have questions about who sees their feedback or how it's used. Static text, the same on every message — the explicit, upfront answer to "who sees this" that the original feature brief flagged as worth surfacing to students rather than leaving unstated.
...
Flags a student whose two most recent mood scores (Quick Pulse or Deep Check-in — one continuous sequence, since Deep Check-in's first question deliberately reuses the same 1-5 scale) are both at least 1.5 points **above** their own trailing average, computed over their last 5 responses — or who has racked up 3 Wellbeing Missing Responses. Reads as "above," not the "below" the name might suggest, because mood is stored 1 = best through 5 = worst (see ADR-0017): a declining student's numbers go *up* toward 5, not down. The name still describes the real-world thing being caught — a drop in wellbeing — not the arithmetic sign.
