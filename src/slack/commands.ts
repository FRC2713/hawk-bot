...
import { isHawkBotAdmin } from "./authz.js";
...
export function registerCommands(app: App): void {
  app.command(
    SLASH_COMMAND,
    async ({ command, ack, respond, client, context }) => {
      await ack();
      ...
      const isAdmin = () =>
        (adminAnswer ??= isHawkBotAdmin(client, command.user_id));

      if (cmd.adminOnly && !(await isAdmin())) {
        await respond({
          response_type: "ephemeral",
          text: `\`${cmd.name}\` is for HawkBot Admins. Ask a coach.`,
        });
        return;
      }
